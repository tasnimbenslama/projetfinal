...
f=fopen(fname,"a+");
if(f!=NULL)
{	while(fscanf(f,"%s %d %s %s %s %d %d %d\n",p.id,&(p.type),p.bloc,p.marque,p.Etat,&(p.d.j),&(p.d.m),&(p.d.a))!=EOF)
	{	
...
if(f==NULL||g==NULL)
{
	...
	while(fscanf(f,"%s %d %s %s %s %d %d %d\n",p.id,&(p.type),p.bloc,p.marque,p.Etat,&(p.d.j),&(p.d.m),&(p.d.a))!=EOF)
	{
		if(strcmp(p.id,u.id)!=0)
			fprintf(g,"%s %d %s %s %d %d %d\n",p.id,p.type,p.bloc,p.marque,p.Etat,p.d.j,p.d.m,p.d.a);
		else
			fprintf(g,"%s %d %s %s %d %d %d\n",p.id,u.type,u.bloc,u.marque,u.Etat,p.d.j,p.d.m,p.d.a);
	}
...
renderer=gtk_cell_renderer_text_new();
	column=gtk_tree_view_column_new_with_attributes(" Etat", renderer,"text",ID, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(liste), column);
	gtk_tree_view_column_set_expand(column,TRUE);
...
{ f = fopen(fname,"a+");
		while(fscanf(f,"%s %s %d %s  %s %d %d %d\n",p.id,p.Etat,&(p.type),p.bloc,p.marque,&(p.d.j),&(p.d.m),&(p.d.a))!=EOF)
	{
...
if(f!=NULL&&g!=NULL){
	...
	{
		if(strcmp(p.id,u.id)!=0)
			fprintf(g,"%s  %s %d %s %s %d %d %d\n",p.id,p.Etat,p.type,p.bloc,p.marque,p.d.j,p.d.m,p.d.a);
	}
...
if(store==NULL)
{
	...
	gtk_tree_view_column_set_expand(column,TRUE);
renderer=gtk_cell_renderer_text_new();
	column=gtk_tree_view_column_new_with_attributes(" Etat", renderer,"text",VALEUR, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(liste), column);
	gtk_tree_view_column_set_expand(column,TRUE);
